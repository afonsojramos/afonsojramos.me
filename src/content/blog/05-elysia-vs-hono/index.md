...
:::note[July 2026 update]
I'm less bullish today. I still use Bun in production and value what it gets right, but I have also encountered memory leaks in long-running workloads. Bun's [account of its rewrite from Zig to Rust](https://bun.com/blog/bun-in-rust) openly describes the memory leaks and crashes that motivated the change. [Zig creator Andrew Kelley disputed Bun creator Jarred Sumner's framing](https://andrewkelley.me/post/my-thoughts-bun-rust-rewrite.html), arguing that the deeper problem was Bun's engineering practices and accumulated technical debt rather than Zig itself; the public exchange also became more personal than the technical disagreement warranted. I don't see Bun's problems as an indictment of Zig, nor do I assume that changing languages automatically fixes the underlying engineering trade-offs. Taken together with my own production experience, the episode reduced my trust in Bun as a runtime for long-lived services. It remains an impressive project that I will continue to use where it fits, but with more caution than this original closing note suggests.
:::
